...
class Receive extends BaseController
{
    ...
    public function index(){
            $data['arr'] = 'Letters Received';
            $data['title'] = 'Form Penerimaan Surat'; 
            $model = new Surat_user_model();
            if (session('role_id') == 3) {
                $data['surats'] = $model->getSuratUserGuru(session('id'));
            } else {
                $data['surats'] = $model->getSuratUser();
            }
            
           
            Services::template('receive/index',$data);
    }
    ...
    public function add(){
            ...
            Services::template('receive/form',$data);
    }
